...
#   in some R versions.
# - As of pipeline v2.1: per-node parameter lists are passed in via
#   `node_params` (built by build_node_params() in 02_params.R). This lets
#   W (wxtnormm/sd) and Z (zxtnormm/sd) vary by region, and is extensible
#   to any other onesim parameter.
# =============================================================================
...
# ── Main simulation ──────────────────────────────────────────────────────────
# node_params: list of per-node parameter lists in V(g)$name order, length
#              vcount(g). Build it with build_node_params(g, base_params)
#              from 02_params.R so that W and Z (and any future per-region
...
simulate_network <- function(g, mix_matrix, node_params,
  ...
  if (length(node_params) != n)
    stop(sprintf("node_params length (%d) != number of nodes (%d). ",
         ...
         "Did you forget build_node_params(g, base_params)?")
  ...
  for (t in seq_len(nseasons)) {
    ...
    # One season of degeneration per node — each node uses its OWN params
